function thermode_test(sub,input_counterbalance_file, run_num, debug)
...
vnames = {'src_subject_id', 'session_id','param_trigger_onset',...
    'eightbit',	'duration',	'temperature',	'plateau',	'sequence',...
...
T.temperature = shuffled.temperature;
T.plateau = shuffled.plateau;
...
for trl = 1:size(countBalMat,1)
  ...
  %end
    ...
    WaitSecs('UntilTime', T.event05_administer_displayonset(trl) + shuffled.duration(trl));
